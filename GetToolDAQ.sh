...
then
    ...
    tar -xzf https://root.cern/download/root_v6.20.08.source.tar.gz
    cd root-6.20.08
    mkdir install
    cd install
    cmake ../ -Dcxx14=ON -Dgdml=ON -Dxml=ON -Dmt=ON -Dkrb5=ON -Dmathmore=ON -Dx11=ON -Dimt=ON -Dtmva=ON -DCMAKE_BUILD_TYPE=RelWithDebInfo -Dpythia6=ON -Dfftw3=ON
    make -j8
    make install
